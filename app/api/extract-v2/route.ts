...
import { extractMemoDataV2 } from '@/lib/openrouter';
...
export async function POST(request: NextRequest) {
  try {
    ...
    // Validate that the memo contains at least one section tag
    if (!body.memo.includes('<SECTION_')) {
      return NextResponse.json(
        { success: false, error: 'Invalid memo format. Expected <SECTION_X> tags.' },
        { status: 400 }
      );
    }
    ...
    console.log(`Processing V2 memo extraction (${body.memo.length} characters)...`);
    const startTime = Date.now();

    const result = await extractMemoDataV2(body.memo, apiKey);
